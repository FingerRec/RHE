...
parser.add_argument('--arch', type=str, default="mpi3d")
...
def main():
    ...
    test_transforms = transforms.Compose([videotransforms.VideoCrop(256)])
        ...
        with torch.no_grad():
            for data in datas:#1 x 3 x 200 x 256 x 256
                ...
                for i in range(args.test_clips):
                    ...
                    main_path, _, _, _ = net(input_var)
                    if output is None:
                        output = main_path.data.cpu().numpy().copy() / args.test_clips / coeff
                    else:
                        output += main_path.data.cpu().numpy().copy() / args.test_clips / coeff
        return output, label
    ...
    video_pred = [np.argmax(x[0]) for x in output]
    np.save("test_output/" + args.dataset + '/' + str(np.mean(cls_acc)*100)[:5] + '_' + "video_pred.npy", video_pred)
    video_labels = [x[1] for x in output]
    np.save("test_output/" + args.dataset + '/' + str(np.mean(cls_acc)*100)[:5] + '_' + "video_labels.npy", video_labels)
    cf = confusion_matrix(video_labels, video_pred).astype(float)
    np.save("test_output/" + args.dataset + '/' + str(np.mean(cls_acc)*100)[:5] + '_' + args.dataset + "_confusion.npy", cf)
    cls_cnt = cf.sum(axis=1)
    cls_hit = np.diag(cf)
    cls_acc = cls_hit / cls_cnt
    print(cls_acc)
    ...
    if args.save_scores is not None:
        ...
        np.savez('test_output/' + args.dataset + '/' + str(np.mean(cls_acc)*100)[:5] + '_' + args.dataset + '_' + args.mode + '_' + args.save_scores, scores=reorder_output, labels=reorder_label)
