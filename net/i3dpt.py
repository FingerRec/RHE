...
class I3D(torch.nn.Module):
    def __init__(self,
        ...
        #====================add for temporal pyramid ================
        #self.tpp_block = TemporalDependBlock(1024,self.num_classes,(1,2,4,8))
        #self.tpp_block_2 = TemporalDependBlock(1024,self.num_classes,(1,2,4,7))
        #====================add for sub block========================
        #self.temporal_sub_block = TemporalSubBlock()
        #====================add for temporal sub block====================
        #self.dropout = TemporalDropoutBlock(dropout_prob)
        #self.dropout = DropBlock3D(block_size=10, drop_prob=0.2)
        #self.dropout = TemporalDropoutBlock3D(drop_prob=0.2)
        #self.dropout = TemporalSameRadioDropoutBlock3D()

        #=======================add for multi frequency================
        '''
        self.s_depend = StraightMultiFrequencyBlock(480, 480, ratio=8, frames=32)
        self.m_depend = StraightMultiFrequencyBlock(832, 832, ratio=8, frames=16)
        self.l_depend = StraightMultiFrequencyBlock(1024, 1024, ratio=8, frames=8)
        '''
        #============================add for TBTAM=====================
        '''
        self.s_depend = TBTAM(480, 480)
        self.m_depend = TBTAM(832, 832)
        self.l_depend = TBTAM(1024, 1024)
        '''
        #=============================add for timeinception==================
        '''
        self.s_depend = TimeInceptionBlock(480, 480)
        self.m_depend = TimeInceptionBlock(832, 832)
        self.l_depend = TimeInceptionBlock(1024, 1024)
        '''
        #==========================add for timeinception2==================
        # self.s_depend = TimeInception2Block(480, 480)
        # self.m_depend = TimeInception2Block(832, 832)
        # self.l_depend = TimeInception2Block(1024, 1024)
    ...
    def forward(self, inp):
        ...
        # out = self.l_depend(out)
        # temporal_path = self.tpp_block(out)
        # temporal_sub_path = self.temporal_sub_block(out)
        # print(temporal_sub_path.size())
        # temporal_sub_path = self.tpp_block_2(temporal_sub_path)
        ...
        out = out.mean(2)
        out_logits = out
        #return F.log_softmax(out, dim=1), F.log_softmax(out_logits, dim=1)
        return F.log_softmax(out, dim=1)
        #out = F.log_softmax(out_logits, dim=1) + F.log_softmax(temporal_path, dim=1) + F.log_softmax(temporal_sub_path, dim=1)
