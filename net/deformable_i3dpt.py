...
# -*- coding: utf-8 -*-
"""
     # @Time    : 2019-07-16 21:12
     # @Author  : Awiny
     # @Site    :
     # @Project : pytorch_i3d
     # @File    : deformable_i3dpt.py
     # @Software: PyCharm
...
from net.deformable_conv.modules import DeformConv
...
class DeformablePooling(torch.nn.Module):
    def __init__(self, kernel_size, stride=None, in_channels=1024):
        super(DeformablePooling, self).__init__()
        self.spatial_pooling = torch.nn.MaxPool3d((1, kernel_size[1], kernel_size[2]), stride=(1, stride[1], stride[2]))
        self.num_deformable_groups = 16
        self.kH, kW = kernel_size[0], 1
        self.deform_conv = torch.nn.Conv2d(in_channels,
        self.num_deformable_groups * 2 * self.kH * kW,
        kernel_size=(self.kH, kW),
        stride=(stride[0], 1),
        padding=0,
        bias=False).cuda()
        self.conv_offset2d = DeformConv(
            in_channels,
            in_channels, (self.kH, kW),
            stride=(stride[0], 1),
            padding=0,
            num_deformable_groups=self.num_deformable_groups).cuda()

    def forward(self, inp):
        deform_branch = self.spatial_pooling(inp)
        b, c, t, h, w = deform_branch.size()
        deform_branch = deform_branch.view(b, c, t, 1)
        deform_offset = self.deform_conv(deform_branch)
        deform_branch = self.conv_offset2d(deform_branch, deform_offset)
        out = deform_branch.unsqueeze(4)
        return out
...
class Deform_I3D(torch.nn.Module):
    def __init__(self,
                 num_classes,
                 modality='rgb',
                 dropout_prob=0,
                 name='inception'):
        super(Deform_I3D, self).__init__()
        ...
        # =======================add for deformable conv===================
        self.deform_pool = DeformablePooling((2, 7, 7), (1, 1, 1), 1024)
