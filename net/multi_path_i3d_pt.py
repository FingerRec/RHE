...
from net.DilationDepend import DilatedDependBlock
...
class I3D(torch.nn.Module):
    def __init__(self,
        ...
        # ===========================add for multi path===========================
        # self.ensemble = torch.nn.Sequential(
        #                                     DilatedDependBlock(256, self.num_classes),
        #                                     DilatedDependBlock(480, self.num_classes),
        #                                     DilatedDependBlock(480, self.num_classes),
        #                                     DilatedDependBlock(512, self.num_classes),
        #                                     DilatedDependBlock(512, self.num_classes),
        #                                     DilatedDependBlock(512, self.num_classes),
        #                                     DilatedDependBlock(528, self.num_classes),
        #                                     DilatedDependBlock(832, self.num_classes),
        #                                     DilatedDependBlock(832, self.num_classes),
        #                                     DilatedDependBlock(832, self.num_classes),
        #                                     DilatedDependBlock(1024, self.num_classes))
        # self.ensemble = torch.nn.Sequential(
        #                                     DilatedDependBlock(528, self.num_classes),
        #                                     DilatedDependBlock(832, self.num_classes),
        #                                     DilatedDependBlock(1024, self.num_classes))
    ...
    def forward(self, inp, drop_flag=True):
        ...
        pool3d_4a = self.maxPool3d_4a_3x3(mixed_3c)
        mixed_4b = self.mixed_4b(pool3d_4a)
        mixed_4c = self.mixed_4c(mixed_4b)
        mixed_4d = self.mixed_4d(mixed_4c)
        mixed_4e = self.mixed_4e(mixed_4d)
        mixed_4f = self.mixed_4f(mixed_4e)
        pool3d_5a = self.maxPool3d_5a_2x2(mixed_4f)
        mixed_5b = self.mixed_5b(pool3d_5a)
        mixed_5c = self.mixed_5c(mixed_5b)
        out = self.avg_pool(mixed_5c)
        out = self.dropout(out)
        out = self.conv3d_0c_1x1_custom(out)
        out = out.squeeze(3)
        out = out.squeeze(3)
        out = out.mean(2)
        # all_levels = [mixed_3b, mixed_3c, pool3d_4a, mixed_4b, mixed_4c, mixed_4d, mixed_4e, mixed_4f, pool3d_5a,
        #               mixed_5b, mixed_5c]
        # all_levels = [mixed_4e, mixed_4f, mixed_5c]
        all_levels = [mixed_4d, mixed_4e, mixed_4f, mixed_5b, mixed_5c]
        return F.log_softmax(out, dim=1), [F.log_softmax(self.ensemble[i](all_levels[i]), dim=1)
