...
class SpatialPyramidPool3D(nn.Module):
    ...
    def forward(self, x):
        ...
        for n in self.out_side:
            ...
            # print(y.size())
            if out is None:
                out = y.view(y.size()[0], y.size()[1], -1)
            else:
                out = torch.cat((out, y.view(y.size()[0], y.size()[1], -1)), 2)
        return out
...
class InceptionI3d(nn.Module):
    ...
    def __init__(self, num_classes=400, spatial_squeeze=True,
        ...
        self.channel_attention = SELayer3D(1024, reduction=32)
    ...
    def forward(self, x):
        ...
        triplet_feature = self.spp(x) #shou be in the format 1024 x 153
        triplet_feature = self.channel_attention(triplet_feature)
        #method1: directly get mean from 1024 channel
