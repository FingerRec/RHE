...
class Unit3D(torch.nn.Module):
    def __init__(self,
        ...
        if self.padding == 'SAME':
            ...
            raise ValueError(
                'padding should be in [VALID|SAME] but got {}'.format(self.padding))
        ...
        if self.use_bn:
            self.batch3d = torch.nn.BatchNorm3d(output_channels)
...
class InceptionI3d(nn.Module):
    ...
    def forward(self, x):
        ...
        #print(x.size()) # 1 x 1024 x 8 x 7 x 7
        #print(self.spp_layer(x).size()) # 1 x 156672
        #print(self.avg_pool(x).size()) # 1 x 1024 x 7 x 1 x 1
        x = self.logits(self.dropout(self.avg_pool(x))) # 1 x 400 x 7 x 1 x 1
        #path_2 = self.spp_layer(x)
        #x = self.logits(self.dropout(self.spp_layer(x)))
        #print(x.size())
        if self._spatial_squeeze:
            logits = x.squeeze(3).squeeze(3) # remove dim whose size is 1 1 x 400 x7
        logits = torch.mean(logits, 2) #1 x 400 x1
        #print(logits)
        if self._num_classes != 400:
