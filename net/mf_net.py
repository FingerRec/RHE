...
try:
    from .basic_models import initializer
except:
    import initializer
...
class MFNET_3D(nn.Module):
    ...
    def __init__(self, num_classes, pretrained=False, **kwargs):
        ...
        #self.spp_layer = SpatialPyramidPool3D(out_side=(1, 2, 4))
        #num_in, num_filter,
        #kernel = (1, 1, 1), pad = (0, 0, 0), stride = (1, 1, 1), g = 1, bias = False):
        #self.path2_de_logits = BN_AC_CONV3D(conv5_num_out, conv5_num_out, kernel=(7, 1, 1),stride=(7, 1, 1), g = 1, bias=False)
        #############
        # Initialization
    ...
    def forward(self, x):
        ...
        h = self.tail(h)
        '''
        path_1 = self.globalpool(h)
        path_2 = self.spp_layer(h)
