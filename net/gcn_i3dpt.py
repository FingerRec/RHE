...
from net.basic_models.TPP import TemporalDependBlock
from net.basic_models.MultiFrequency import StraightMultiFrequencyBlock, TBTAM, TimeInceptionBlock, TimeInception2Block
from net.basic_models.TemporalSubBlock import TemporalSubBlock
from net.basic_models.TemporalDropoutBlock import TemporalDropoutBlock, DropBlock3D, TemporalDropoutBlock3D, TemporalSameRadioDropoutBlock3D
from net.gcn.gcn_layer import GraphConvolution
from net.gcn.adjcent_matrix import define_matrix
from net.gcn.label_matrix import label_matrix
...
class GCNBlock(nn.Module):
    def __init__(self, num_classes):
        ...
        # ===========================gcn module==================================
        self.gc1 = GraphConvolution(num_classes, 512)
        self.gc2 = GraphConvolution(512, num_classes)
        self.relu = nn.LeakyReLU(0.2)
        self.word_vec = label_matrix(num_classes).cuda()
        # self.adj = define_matrix(self.num_classes).cuda()

    def forward(self, inp, adj):
        x = self.gc1(self.word_vec, adj)
        x = self.relu(x)
        x = self.gc2(x, adj)
        x = x.transpose(0, 1)
        return torch.matmul(inp, x)


class GCN_I3D(torch.nn.Module):
    def __init__(self,
                 num_classes,
                 modality='rgb',
                 dropout_prob=0,
                 name='inception'):
        super(GCN_I3D, self).__init__()
        ...
        # ==========================add for multi path==========================]
        self.s_depend = MultiDependBlock(480, self.num_classes)
        self.m_depend = MultiDependBlock(832, self.num_classes)
        self.l_depend = MultiDependBlock(1024, self.num_classes)
        self.s_gcn = GCNBlock(self.num_classes)
        self.m_gcn = GCNBlock(self.num_classes)
        self.l_gcn = GCNBlock(self.num_classes)
        '''
        #self.gc1 = GraphConvolution(1024, num_classes)
        self.adj = define_matrix(8).cuda()
        self.gcn_spatial_avg_pool = torch.nn.AdaptiveAvgPool3d((8,1,1))
        self.gcn_spatial_max_pool = torch.nn.AdaptiveMaxPool3d((8,1,1))
        self.gcn_temporal_max_pool = torch.nn.AdaptiveMaxPool1d((1))
        self.gcn_temporal_avg_pool = torch.nn.AdaptiveAvgPool1d((1))
        self.gcn_bn = torch.nn.BatchNorm1d(num_classes)
        '''
    def forward(self, inp, adj):
        ...
        '''
        b, c, t, h, w = out.size()
        gcn_path = self.gcn_spatial_max_pool(out).squeeze(3).squeeze(3)/2 + self.gcn_spatial_avg_pool(out).squeeze(3).squeeze(3)/2
        gcn_path = gcn_path.transpose(1, 2)
        gcn_out = torch.zeros(b, t, self.num_classes).cuda()
        for i in range(b):
            #gcn_out[i, :, :] = self.gc1(gcn_path[i, :, :], self.adj)
            gcn_out[i,:,:] = self.gc2(F.relu(self.gc1(gcn_path[i, :, :], self.adj)), self.adj)
        gcn_out = gcn_out.transpose(1, 2)
        gcn_out = gcn_out[:,:,t-1].view(b, -1)
        #gcn_out = self.gcn_temporal_max_pool(gcn_out).view(b, -1)
        #gcn_out = self.gcn_bn(gcn_out)
        #print(gcn_out)
        '''
        ...
        path_s = self.s_gcn(path_s, adj)
        path_m = self.m_gcn(path_m, adj)
        path_l = self.l_gcn(path_l, adj)
        return F.log_softmax(out, dim=1), F.log_softmax(path_s, dim=1), F.log_softmax(path_m, dim=1), \
               F.log_softmax(path_l, dim=1)
