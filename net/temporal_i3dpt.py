...
class TemporalMixed(torch.nn.Module):
    def __init__(self, in_channel, out_channel, compress=1):
        """

        :param in_channel: input channels
        :param out_channel: compressed channels
        """
        super(TemporalMixed, self).__init__()
        self.compress_rate = compress
        self.channel_compress = Unit3Dpy(in_channel, out_channel, kernel_size=(1, 1, 1), padding='VALID')
        self.branch_0 = nn.Sequential(Unit3Dpy(in_channels=out_channel, out_channels=out_channel//self.compress_rate,
                                                kernel_size=(1, 1, 1), padding='SAME'),
                                      nn.BatchNorm3d(out_channel//self.compress_rate)
                                      )
        self.branch_1 = nn.Sequential(Unit3Dpy(in_channels=out_channel, out_channels=out_channel//self.compress_rate,
                                                kernel_size=(3, 1, 1), padding='SAME'),
                                      nn.BatchNorm3d(out_channel // self.compress_rate)
                                      )
        self.branch_2 = nn.Sequential(Unit3Dpy(in_channels=out_channel, out_channels=out_channel//self.compress_rate,
                                                kernel_size=(5, 1, 1), padding='SAME'),
                                      nn.BatchNorm3d(out_channel // self.compress_rate)
                                      )
        self.branch_3 = nn.Sequential(Unit3Dpy(in_channels=out_channel, out_channels=out_channel//self.compress_rate,
                                                kernel_size=(7, 1, 1), padding='SAME'),
                                      nn.BatchNorm3d(out_channel // self.compress_rate)
                                      )
        self.channel_uncompress = Unit3Dpy(in_channels=out_channel//self.compress_rate*4, out_channels=in_channel, kernel_size=(1, 1, 1), padding='VALID')

    def forward(self, inp):
        residual = inp
        inp = self.channel_compress(inp)
        out_0 = self.branch_0(inp)
        out_1 = self.branch_1(inp)
        out_2 = self.branch_2(inp)
        out_3 = self.branch_3(inp)
        out = torch.cat((out_0, out_1, out_2, out_3), 1)
        out = self.channel_uncompress(out)
        return out + residual
...
class I3D(torch.nn.Module):
    def __init__(self,
        ...
        self.maxPool3d_5a_2x2 = MaxPool3dTFPadding(
            kernel_size=(1, 2, 2), stride=(1, 2, 2), padding='SAME')
        ...
        self.conv3d_0c_1x1_custom_1 = Unit3Dpy(
            in_channels=1024,
        ...
        self.tpp_1 = MultiDependBlock(1024, num_classes)
    ...
    def forward(self, inp):
        ...
        temporal_path = self.tpp_1(out)
        out = self.avg_pool(out)
        out = self.dropout(out)
        out = self.conv3d_0c_1x1_custom_1(out)
        out = out.squeeze(3)
        out = out.squeeze(3)
        out = out.mean(2)
        return F.log_softmax(out, dim=1) + F.log_softmax(temporal_path, dim=1)
...
class I3D(torch.nn.Module):
    def __init__(self,
        ...
        conv3d_1a_7x7 = Unit3Dpy(
            out_channels=64//self.compress_rate,
            in_channels=in_channels,
            kernel_size=(1, 7, 7),
            stride=(1, 2, 2),
            padding='SAME')
        # 1st conv-pool
        self.conv3d_1a_7x7_custom = conv3d_1a_7x7
        self.maxPool3d_2a_3x3_custom = MaxPool3dTFPadding(
            kernel_size=(1, 3, 3), stride=(1, 2, 2), padding='SAME')
        # conv conv
        conv3d_2b_1x1 = Unit3Dpy(
            out_channels=64//self.compress_rate,
            in_channels=64//self.compress_rate,
            kernel_size=(1, 1, 1),
            padding='SAME')
        self.conv3d_2b_1x1_custom = conv3d_2b_1x1
        conv3d_2c_3x3 = Unit3Dpy(
            out_channels=192//self.compress_rate,
            in_channels=64//self.compress_rate,
            kernel_size=(3, 3, 3),
            padding='SAME')
        self.conv3d_2c_3x3_custom = conv3d_2c_3x3 #here padding = 1 may influence the result
        self.maxPool3d_3a_3x3_custom = MaxPool3dTFPadding(
            kernel_size=(1, 3, 3), stride=(1, 2, 2), padding='SAME')

        # Mixed_3b
        self.mixed_3b_custom = Mixed(192//self.compress_rate, [64//self.compress_rate, 96//self.compress_rate, 128//self.compress_rate, 16//self.compress_rate, 32//self.compress_rate, 32//self.compress_rate])
        self.mixed_3c_custom = Mixed(256//self.compress_rate, [128//self.compress_rate, 128//self.compress_rate, 192//self.compress_rate, 32//self.compress_rate, 96//self.compress_rate, 64//self.compress_rate])

        self.maxPool3d_4a_3x3_custom = MaxPool3dTFPadding(
            kernel_size=(1, 3, 3), stride=(1, 2, 2), padding='SAME')

        # Mixed 4
        self.mixed_4b_custom = Mixed(480//self.compress_rate, [192//self.compress_rate, 96//self.compress_rate, 208//self.compress_rate, 16//self.compress_rate, 48//self.compress_rate, 64//self.compress_rate])
        self.mixed_4c_custom = Mixed(512//self.compress_rate, [160//self.compress_rate, 112//self.compress_rate, 224//self.compress_rate, 24//self.compress_rate, 64//self.compress_rate, 64//self.compress_rate])
        self.mixed_4d_custom = Mixed(512//self.compress_rate, [128//self.compress_rate, 128//self.compress_rate, 256//self.compress_rate, 24//self.compress_rate, 64//self.compress_rate, 64//self.compress_rate])
        self.mixed_4e_custom = Mixed(512//self.compress_rate, [112//self.compress_rate, 144//self.compress_rate, 288//self.compress_rate, 32//self.compress_rate, 64//self.compress_rate, 64//self.compress_rate])
        self.mixed_4f_custom = Mixed(528//self.compress_rate, [256//self.compress_rate, 160//self.compress_rate, 320//self.compress_rate, 32//self.compress_rate, 128//self.compress_rate, 128//self.compress_rate])

        self.maxPool3d_5a_2x2_custom = MaxPool3dTFPadding(
            kernel_size=(1, 2, 2), stride=(1, 2, 2), padding='SAME')

        # Mixed 5
        self.mixed_5b_custom = Mixed(832//self.compress_rate, [256//self.compress_rate, 160//self.compress_rate, 320//self.compress_rate, 32//self.compress_rate, 128//self.compress_rate, 128//self.compress_rate])
        self.mixed_5c_custom = Mixed(832//self.compress_rate, [384//self.compress_rate, 192//self.compress_rate, 384//self.compress_rate, 48//self.compress_rate, 128//self.compress_rate, 128//self.compress_rate])

        # self.avg_pool = torch.nn.AvgPool3d((1, 7, 7), (1, 1, 1))
        self.avg_pool_custom = torch.nn.AdaptiveAvgPool3d((1, 1, 1))
        self.dropout_custom = torch.nn.Dropout(dropout_prob)
        self.conv3d_0c_1x1_custom_custom = Unit3Dpy(
            in_channels=1024//self.compress_rate,
            out_channels=self.num_classes,
            kernel_size=(1, 1, 1),
            activation=None,
            use_bias=False,
            use_bn=False)
        self.softmax_custom = torch.nn.Softmax(1)

        self.tpp_custom = MultiDependBlock(1024//self.compress_rate, num_classes)
        # =====================Temporal inception =====================================
        self.temporal_mixed_3 = TemporalMixed(480//self.compress_rate, 480//self.compress_rate)
        self.temporal_mixed_4 = TemporalMixed(832//self.compress_rate, 832//self.compress_rate)
        self.temporal_mixed_5 = TemporalMixed(1024//self.compress_rate, 1024//self.compress_rate)

    def forward(self, inp):
        out = self.conv3d_1a_7x7_custom(inp)
        out = self.maxPool3d_2a_3x3_custom(out)
        out = self.conv3d_2b_1x1_custom(out)
        out = self.conv3d_2c_3x3_custom(out)
        out = self.maxPool3d_3a_3x3_custom(out)
        out = self.mixed_3b_custom(out)
        out = self.mixed_3c_custom(out)
        # ====temporal Mixed=====
        out = self.temporal_mixed_3(out)
        out = self.maxPool3d_4a_3x3_custom(out)
        out = self.mixed_4b_custom(out)
        out = self.mixed_4c_custom(out)
        out = self.mixed_4d_custom(out)
        out = self.mixed_4e_custom(out)
        out = self.mixed_4f_custom(out)
        out = self.temporal_mixed_4(out)
        out = self.maxPool3d_5a_2x2_custom(out)
        out = self.mixed_5b_custom(out)
        out = self.mixed_5c_custom(out)
        out = self.temporal_mixed_5(out)
        temporal_path = self.tpp_custom(out)
        out = self.avg_pool_custom(out)
        out = self.dropout_custom(out)
        out = self.conv3d_0c_1x1_custom_custom(out)
        out = out.squeeze(3)
        out = out.squeeze(3)
        out = out.mean(2)
        return F.log_softmax(out, dim=1) + F.log_softmax(temporal_path, dim=1)
