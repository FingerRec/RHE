...
# -*- coding: utf-8 -*-
"""
     # @Time    : 2019-03-16 09:38
     # @Author  : Awiny
     # @Site    :
     # @Project : pytorch_i3d
     # @File    : pure_triplet_dataset.py
     # @Software: PyCharm
...
class PureTripletDataSet(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if not self.test_mode:
            ...
            data_ni, _ = _get_data(record_3, _data_augment=True)

            return data_ai, data_ni, data_ap, label
        else:
            data_ai, label = _get_data(record, _data_augment=False)
            ...
            while 1==1:
                ...
                if record_2.path != record.path:
                    break
            data_ap, _ = _get_data(record_2, _data_augment=False)
            nearest_class_label = self.simiar_list[record.label]
            nearest_class_action = self.parse_dict[nearest_class_label[0]]
            index_3 = nearest_class_action[randint(0, len(nearest_class_action))]
            record_3 = self.video_list[index_3]
            data_ni, _ = _get_data(record_3, _data_augment=False)
