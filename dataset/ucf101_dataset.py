...
class I3dDataSet(data.Dataset):
    ...
    def _load_image(self, directory, idx):
        ...
        # directory = "/home/wjp/Desktop/disk2_6T/DataSet/interplate_5_hmdb51_imgs/" + directory.strip().split(' ')[0].split('/')[-1]
        if self.modality == 'rgb' or self.modality == 'RGBDiff' or self.modality == 'RGB':
            ...
            if self.dataset == 'hmdb51':
                ...
                width, height = img.size
                # print(width, height)
                img = img.resize((int(64/height*width), 64), Image.BILINEAR)
                return [img]
            else:
                return [img]
    ...
    def __getitem__(self, index):
        ...
        if not self.test_mode:
            ...
            # data, label = self.get_test(record, segment_indices, data_augment=False)
            # data = 2 * (data / 255) - 1
            data = self.transform(data)
            if type(data) == list and len(data) > 1:
                new_data = list()
    ...
    #     if not self.full_video:
    #         for i in range(self.new_length):
    #             seg_imgs = self._load_image(record.path, index)
    #             images.extend(seg_imgs)
    #             if p < (record.num_frames - self.stride - 5) * 5:
    #                 p += self.stride * 5
    ...
    # def get_test(self, record, indices, data_augment=False, side_length=224, is_numpy=True):
    #     '''
    #     get num_segments data
    #     '''
    #     images = list()
